...
- Bei sichtbaren oder interaktiven Änderungen endet die autonome Lieferung standardmäßig am Gate `PR mit CI grün`. Vor dem Merge nach `dev` erhält der Nutzer eine selbst testbare Version und gibt sie ausdrücklich frei; nur eine ausdrückliche Anweisung darf dieses Human-in-the-loop-Gate für einen Scope aufheben.
- Das Testpaket enthält eine verifizierte PR-Preview-URL oder, solange keine Preview-Infrastruktur verfügbar ist, einen verifizierten lokalen Testzugang, außerdem einen kurzen Ergebnisüberblick, drei bis sechs konkrete manuelle Testschritte, bekannte Grenzen sowie Commit-, PR- und CI-Nachweise. Bei visuellen Änderungen gehört nach Möglichkeit ein aktueller Screenshot dazu.
- Der Koordinationstask präsentiert das Testpaket, sammelt das Nutzerfeedback und gibt es an den Umsetzungstask zurück. Nach Korrekturen wird dasselbe Testgate wiederholt. Erst nach Nutzerfreigabe wird nach `dev` gemergt; anschließend werden der integrierte Stand und der erreichbare Testzugang erneut kurz geprüft.
